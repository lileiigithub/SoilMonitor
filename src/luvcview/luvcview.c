...
int main(int argc, char *argv[])
{
    ...
    printf("Test SDL capabilities end!\n");
	...
	control_s.value= 0;
	ioctl(videoIn->fd, VIDIOC_S_CTRL, &control_s);
	control_s.id = V4L2_CID_WHITE_BALANCE_TEMPERATURE;
	control_s.value= 3000;
	ioctl(videoIn->fd, VIDIOC_S_CTRL, &control_s);
	// set exposure
	...
	control_s.value= 2000;
	ioctl(videoIn->fd, VIDIOC_S_CTRL, &control_s);
	
	// get WHITE_BALANCE_TEMPERATURE
	constrol_get.id = V4L2_CID_WHITE_BALANCE_TEMPERATURE;
	ioctl(videoIn->fd, VIDIOC_G_CTRL, &constrol_get);
	printf("V4L2_CID_WHITE_BALANCE_TEMPERATURE value: %d\n",constrol_get.value);
	// get EXPOSURE_ABSOLUTE
	constrol_get.id = V4L2_CID_EXPOSURE_ABSOLUTE;
	ioctl(videoIn->fd, VIDIOC_G_CTRL, &constrol_get);
